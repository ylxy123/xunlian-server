...
model_message_dict = {
    ...
    'user': {'model': User, 'count': 200,
             'message': _(
                 'The community version supports up to 200 users. If you need more users, please contact us (https://fit2cloud.com/).')}
}
    ...
    def valid(self, is_valid=True):
        ...
        if not is_license_valid:
            if self.data.get('valid_count') != model_value.get('count'):
                print(self.data.get('valid_count'), model_value.get('count'))
                raise AppApiException(400, model_value.get('message'))
            if QuerySet(model_value.get('model')).count() >= model_value.get('count'):
                print(QuerySet(model_value.get('model')).count(), model_value.get('count'))
                raise AppApiException(400, model_value.get('message'))
        return True
