...
class ChatInfo:
    ...
    def append_chat_record(self, chat_record: ChatRecord, client_id=None, client_ip="append_chat_record"):
        ...
        if self.application.id is not None:
            ...
            if not QuerySet(Chat).filter(id=self.chat_id).exists():
                Chat(id=self.chat_id, application_id=self.application.id, abstract=chat_record.problem_text[0:1024],
                     client_id=client_id, update_time=datetime.now(), client_ip=client_ip).save()
            else:
                Chat.objects.filter(id=self.chat_id).update(update_time=datetime.now(), client_ip=client_ip)
            # 插入会话记录
            chat_record.save()


def get_post_handler(chat_info: ChatInfo, client_ip="get_post_handler"):
    class PostHandler(PostResponseHandler):
        ...
        def handler(self,
            ...
            chat_info.append_chat_record(chat_record, client_id, client_ip)
            # 重新设置缓存
...
class OpenAIChatSerializer(serializers.Serializer):
    ...
    def generate_chat(chat_id, application_id, message, client_id):
        ...
        if chat is None:
            # 生成對話並插入
            Chat(id=chat_id, application_id=application_id, abstract=message[0:1024], client_id=client_id, client_ip="new").save()
        return chat_id
    ...
    def chat(self, instance: Dict, with_valid=True):
        ...
        ).chat(base_to_response=OpenaiToResponse(), client_ip="2222")
...
class ChatMessageSerializer(serializers.Serializer):
    ...
    def chat_simple(self, chat_info: ChatInfo, base_to_response, client_ip="chat_simple"):
        ...
        # 构建运行参数
        params = chat_info.to_pipeline_manage_params(message, get_post_handler(chat_info, client_ip),
                                                     exclude_paragraph_id_list,
    ...
    def chat(self, base_to_response: BaseToResponse = SystemToResponse(), client_ip="222"):
        ...
        if chat_info.application.type == ApplicationTypeChoices.SIMPLE:
            ...
            return self.chat_simple(chat_info, base_to_response, client_ip)
        else:
