...
#     @action(methods=['POST'], detail=False)
#     def post(self, request: Request):
#         user = self.get_user_inner(request.data)
#         token = user_cache.get(user)
#
#         return result.success(token)
#
#     @staticmethod
#     def get_user_inner(data):
#         username = data.get("username")
#         user = QuerySet(UserModel).filter(Q(username=username) | Q(email=username)).first()
#         return user
#
#     class Meta:
#         model = UserModel
